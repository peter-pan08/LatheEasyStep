...
def test_emit_line_numbers_disabled_removes_n_prefix(tmp_path):
    ...
    # ensure no line begins with an N-number prefix
    assert not any(re.match(r"^\s*[Nn]\d+\b", l) for l in lines)
    # G1 may appear inside contour subroutines (for G71/G72 cycles) - that's OK.
    # But no G0 should appear in a contour-only program (no cutting ops).
    assert not any(re.match(r"^\s*G0\b", l) for l in lines)

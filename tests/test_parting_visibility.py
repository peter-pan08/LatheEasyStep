...
def test_parting_visibility_rough_shows_widgets():
    ...
    # finish allowances (visible only when roughing)
    h.label_parting_finish_allow_x = BoolWidget()
    h.parting_finish_allow_x = BoolWidget()
    h.label_parting_finish_allow_z = BoolWidget()
    h.parting_finish_allow_z = BoolWidget()
    ...
    assert h.parting_finish_allow_x.visible is True
    assert h.parting_finish_allow_z.visible is True
...
def test_parting_visibility_finish_hides_widgets():
    h = object.__new__(HandlerClass)
    h.parting_mode = FakeCombo(1, "finish")  # 1 = Schlichten / Finish
    ...
    h.label_parting_slice_step = BoolWidget()
    h.parting_slice_step = BoolWidget()
    # finish allowances (hidden when finishing)
    h.label_parting_finish_allow_x = BoolWidget()
    h.parting_finish_allow_x = BoolWidget()
    h.label_parting_finish_allow_z = BoolWidget()
    h.parting_finish_allow_z = BoolWidget()
    ...
    assert h.parting_finish_allow_x.visible is False
    assert h.parting_finish_allow_z.visible is False
    # slice_step should still be hidden
    assert h.parting_slice_step.visible is False


def test_parting_visibility_rough_finish_shows_rough_widgets():
    h = object.__new__(HandlerClass)
    h.parting_mode = FakeCombo(2, "rough_finish")
